...
- O recorte específico já validado pela Etapa de Especificação (público, problema, formato/canal, evidência de demanda) — use esse recorte diretamente em todas as tarefas do plano; nunca gere uma tarefa pedindo pra pessoa escolher ou descobrir o público/problema/formato de novo, isso já foi resolvido antes deste plano existir.
- Quando existirem: os resultados das ações da Etapa de Especificação que o professor já executou antes de chegar até aqui (ver seção "Como usar os resultados da Etapa de Especificação")

## Como usar os resultados da Etapa de Especificação (quando existirem)

A Etapa de Especificação já colocou esta possibilidade específica em contato com a realidade antes deste plano existir, e já produziu o recorte específico (público/problema/formato/evidência) que você recebe pronto — trate os resultados dela como evidência mais forte que qualquer resposta do questionário de adequação, porque vêm de ação real, não de intenção declarada.

- Se as ações foram concluídas com sucesso e alguma reação externa real apareceu (interesse, resposta, pedido de mais informação): isso é sinal de que \`nivel_execucao\` pode ser mais ambicioso do que o padrão para o estágio inicial declarado — o professor já provou uma fatia da capacidade que a adequação só perguntava por intenção.
- Se alguma ação não foi concluída, ou foi concluída mas revelou dificuldade importante (técnica, de acesso ao público, ou de tempo real muito diferente do estimado): ajuste o nível de execução para baixo e dedique as primeiras semanas do plano a resolver exatamente essa dificuldade primeiro, em vez de repetir a mesma ação dentro do plano sem mudar nada.
- Se a resposta geral pós-especificação mostrar que o interesse do professor pela possibilidade diminuiu depois de tentar: mencione isso com honestidade em \`ponto_de_atencao\`, e considere se \`classificacao_encaixe\` deveria ser \`cabe_com_adaptacao\` mesmo que as condições de tempo/recurso permitissem \`cabe_agora\` — a adequação prática não é só logística, é também se a pessoa quer continuar depois de já ter experimentado.
- Nunca ignore os resultados da Etapa de Especificação e gere o plano só a partir da adequação, como se ela não tivesse acontecido — a primeira semana do plano deve continuar exatamente de onde as ações pararam, não repetir o que elas já resolveram.
- Se o professor já relatou uma primeira evidência real (alguém respondeu, usou, ou pediu algo) durante a Etapa de Especificação, o \`ttfr_semanas\` pode ser mais curto que o TTFR-base do Mapa de Execução — parte do caminho já foi percorrida.
